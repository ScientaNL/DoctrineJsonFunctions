<?php

declare(strict_types=1);

namespace Scienta\DoctrineJsonFunctions\Tests\Integration\Functions\Sqlite;

use Scienta\DoctrineJsonFunctions\Tests\Integration\SqliteIntegrationTestCase;

class JsonInsertTest extends SqliteIntegrationTestCase
{
    public function testInsertsNewKey(): void
    {
        $result = $this->entityManager->createQuery(
            "SELECT JSON_INSERT('{\"a\":1}', '$.b', 2) AS val
             FROM Scienta\\DoctrineJsonFunctions\\Tests\\Entities\\Blank b"
        )->getSingleScalarResult();

        $decoded = json_decode((string) $result, true);
        $this->assertEquals(1, $decoded['a']);
        $this->assertEquals(2, $decoded['b']);
    }

    public function testDoesNotOverwriteExistingKey(): void
    {
        $result = $this->entityManager->createQuery(
            "SELECT JSON_INSERT('{\"a\":1}', '$.a', 99) AS val
             FROM Scienta\\DoctrineJsonFunctions\\Tests\\Entities\\Blank b"
        )->getSingleScalarResult();

        $decoded = json_decode((string) $result, true);
        $this->assertEquals(1, $decoded['a']);
    }
}
